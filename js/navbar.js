...
//   v2.2 — Arctic Clarity: renderUserBadge() updated to match new
//           light-mode sidebar. Avatar uses square border-radius,
//           gradient matches --blue token. Name/role text uses ink
//           colours instead of white (sidebar is now white not dark).
//           Logout button styled to ghost style.
// ============================================================
//
...
const Session = {
  ...
  renderUserBadge(containerId) {
    ...
    // v2.2 Arctic Clarity: light sidebar — ink text, square avatar, ghost logout
    c.innerHTML = `
      <div style="display:flex;align-items:center;gap:10px;padding:8px;border-radius:6px;transition:background .15s;cursor:default"
           onmouseover="this.style.background='rgba(11,25,41,0.04)'" onmouseout="this.style.background='transparent'">
        <div style="background:linear-gradient(135deg,#1D5FD4,#0D3E9A);color:#fff;width:32px;height:32px;border-radius:8px;
                    flex-shrink:0;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:12px;font-family:'Cabinet Grotesk',sans-serif">
          ${(this._ctx.fullName || '?').charAt(0).toUpperCase()}
        </div>
        <div style="flex:1;min-width:0">
          <div style="font-weight:600;color:#0B1929;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${this._ctx.fullName || this._ctx.email}</div>
          <div style="color:rgba(11,25,41,0.4);font-size:10px;font-weight:500;letter-spacing:.04em;text-transform:uppercase">${this._ctx.primaryRole || this._ctx.userType}</div>
        </div>
        <button onclick="Session.logout()"
          style="background:transparent;border:1.5px solid rgba(11,25,41,0.08);color:rgba(11,25,41,0.4);
                 cursor:pointer;padding:4px 7px;border-radius:4px;font-size:11px;flex-shrink:0;
                 transition:all .15s;font-family:'Instrument Sans',sans-serif"
          onmouseover="this.style.background='rgba(11,25,41,0.06)';this.style.color='#0B1929'"
          onmouseout="this.style.background='transparent';this.style.color='rgba(11,25,41,0.4)'"
